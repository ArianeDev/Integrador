export function loadProducts(productList, load) {
  ...
  productList.forEach((produto) => {
    ...
    const html = `
      <a href="product.html">
        <button id="${produto.codigoProduto}" class="product-card-btn">
          <div class="product-card idprod" id="${produto.codigoProduto}">
            <div class="img" id="${produto.codigoProduto}">
              <img id="${produto.codigoProduto}" src="${produto.imagemProduto.img1}" alt="${produto.tituloProduto}" />
            </div>
            <div class="product-card-info-container" id="${produto.codigoProduto}">
              <h2 class="product-card-title" title="${produto.tituloProduto}" id="${produto.codigoProduto}">${produto.tituloProduto}</h2>
              <h3 class="product-card-price" id="${produto.codigoProduto}">R$ ${produto.preco.toFixed(2)}</h3>
              <h4 class="product-card-installment" id="${produto.codigoProduto}">10x de R$ ${valParcela} s/juros</h4>
            </div>
          </div>
        </button>
      </a>`;
...
const priceTwo = document.querySelector(".autor_container")
priceTwo.children[0].innerText = `Autor(a): ${produto.autora}`
priceTwo.children[1].innerText = `Sobre: ${produto.autoraSobre}`
